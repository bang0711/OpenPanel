...
```bash
docker run --rm -v "$PWD:/output" open-panel:1.2.0 install
# set DATABASE_URL in the generated .env (bring your own Postgres) — or pass it:
#   docker run --rm -e DATABASE_URL=postgresql://… -v "$PWD:/output" open-panel:1.2.0 install
docker compose up -d
...
```bash
docker build -t open-panel:1.2.0 .
```
...
```bash
docker build -t open-panel:1.2.0 .   # build first — the stack won't pull it
