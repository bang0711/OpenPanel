...
#   docker run --rm -e DATABASE_URL=postgresql://… -v "$PWD:/output" open-panel:1.0.1 install
docker compose up -d
...
```bash
docker build -t open-panel:1.0.1 .
```
...
```bash
docker build -t open-panel:1.0.1 .   # build first — the stack won't pull it
